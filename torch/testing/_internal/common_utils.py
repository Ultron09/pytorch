...
def periodic(fn):
    ...
    Replaces @slowTest rather than combining with it: periodic CI does not set
    PYTORCH_TEST_WITH_SLOW, so a test marked both would silently never run.
    """
    if fn.__dict__.get('slow_test', False):
        raise RuntimeError(f"@periodic replaces @slowTest; remove @slowTest from {fn.__name__}")
...
def slowTest(fn):
    if fn.__dict__.get('periodic_test', False):
        raise RuntimeError(f"@periodic replaces @slowTest; remove @slowTest from {fn.__name__}")
...
def check_if_enable(test: unittest.TestCase):
    ...
    if any(matches_test(x) for x in slow_tests_dict):
        ...
        # A @periodic test dynamically marked slow still runs in periodic CI,
        # which does not set PYTORCH_TEST_WITH_SLOW.
        periodic_enabled = TEST_WITH_PERIODIC and test_fn.__dict__.get('periodic_test', False)
