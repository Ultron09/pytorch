...
def _check_cpu_mxfp(
    ...
    scale_b_shape = (
        (mat2.size(1), k_blocks) if v2_scale_b_layout else (k_blocks, mat2.size(1))
    )
    torch._check_value(
        scale_b.dim() == 2,
    ...
    torch._check_value(
        (scale_b.size(0) == scale_b_shape[0])
        & (scale_b.size(1) == scale_b_shape[1]),
        lambda: f"MXFP scale_b must have shape {scale_b_shape}, got {scale_b.shape}",
    )
    torch._check_value(
    ...
    if bias is not None:
        ...
        torch._check_value(
            bias.dtype in (torch.bfloat16, torch.float32),
            lambda: (
                f"CPU MXFP bias must have dtype BFloat16 or Float32, got {bias.dtype}"
            ),
        )
    return torch.empty(
...
@register_meta([aten._scaled_mm.default, aten._scaled_mm.out])
@out_wrapper(exact_dtype=True)
def meta_scaled_mm(
    self: torch.Tensor,
    mat2: torch.Tensor,
...
@register_meta([aten._scaled_mm_v2.default, aten._scaled_mm_v2.out])
@out_wrapper(exact_dtype=True)
def meta_scaled_mm_v2(
    ...
    if _is_cpu_mxfp(self, *scale_a, *scale_b):
        ...
        return _check_cpu_mxfp(
            ...
            bias,
            out_dtype,
            v2_scale_b_layout=True,
        )

    # The structured C++ meta sizes its output via IntArrayRef, which
    # specializes symbolic dims under fake-tensor tracing. Keep symbolic shape
    # inference here for non-MXFP configurations.
    torch._check(
        self.dim() == 2 and mat2.dim() == 2,
        lambda: f"Inputs must be 2D but got self.dim()={self.dim()} and mat2.dim()={mat2.dim()}",
    )
