...
class TestFP8Lowering(TestCase):
    ...
    @onlyOn(["cpu"])
    @parametrize("api,mx_format", (("v1", "mxfp4"), ("v2", "mxfp8")))
    def test_cpu_mxfp_fullgraph(self, device, api, mx_format):
        ...
        scale_b = torch.full((groups, n), 127, dtype=torch.uint8, device=device).view(
            torch.float8_e8m0fnu
        )
        ...
        if api == "v1":
            ...
            args = (a, b, scale_a, scale_b.t().contiguous(), bias)
