...
def _make_mxfp_inputs(
    ...
    *,
    ...
    scale_b = _e8m0(
        torch.randint(120, 135, (groups, n), device=device, generator=generator)
    )
...
class TestScaledMatmulCPU(TestCase):
    ...
    def test_mxfp_numerics(self, device, mx_format, out_dtype):
        a, b, scale_a, scale_b = _make_mxfp_inputs(mx_format, device)
        bias = torch.randn(b.size(1), device=device, dtype=out_dtype)
        expected = (
        ...
        tolerance = 2e-5 if out_dtype == torch.float32 else 0
        self.assertEqual(v1, expected, atol=tolerance, rtol=tolerance)
    ...
    def test_mxfp_encodings(self, device):
        ...
        scale_b = _e8m0(torch.full((1, 2), 127, device=device))
        actual = torch._scaled_mm(a, b, scale_a, scale_b, out_dtype=torch.float32)
    ...
    def test_mxfp_validation_parity(self, device, case, error_type, error_regex):
        ...
        def invoke(a, b, scale_a, scale_b):
            ...
            if case == "bias_dtype":
                bias = torch.zeros(b.size(1), dtype=torch.float16, device=device)
                return _scaled_mm_v2(a, b, scale_a, scale_b, bias, torch.float32)
            if case == "swizzle":
                return _scaled_mm_v2(
                    a,
        ...
        logs = (
            self.assertLogs("torch._subclasses.fake_tensor", level="ERROR")
            if error_type is ValueError
            else contextlib.nullcontext()
        )
        with mode, logs, self.assertRaisesRegex(error_type, error_regex):
            invoke(*fake_inputs)
